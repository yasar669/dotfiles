...
# =======================================================
# BOLUM 6: MUM BIRLESTIRICI — WSS tick -> OHLCV
# =======================================================

_MUM_BIRLESTIRICI="${BORSA_KLASORU}/tarama/_mum_birlestirici.py"
_MUM_BIRLESTIRICI_PID="/tmp/borsa/_takip/birlestirici.pid"

# -------------------------------------------------------
# mum_birlestirici_baslat
# Arka planda mum birlestirici baslatir.
# Tick dosyalarindan okuyup OHLCV mumlarina cevirir.
# -------------------------------------------------------
mum_birlestirici_baslat() {
    _ohlcv_python_bul || {
    ...
    if [[ ! -f "$_MUM_BIRLESTIRICI" ]]; then
        echo "HATA: $_MUM_BIRLESTIRICI dosyasi bulunamadi" >&2
        return 1
    fi

    # Zaten calisiyor mu kontrol et
    if [[ -f "$_MUM_BIRLESTIRICI_PID" ]]; then
        local pid
        pid=$(cat "$_MUM_BIRLESTIRICI_PID" 2>/dev/null)
        if [[ -n "$pid" ]] && kill -0 "$pid" 2>/dev/null; then
            echo "Mum birlestirici zaten calisiyor (PID: $pid)"
            return 0
        fi
        rm -f "$_MUM_BIRLESTIRICI_PID"
    fi

    # Supabase ortam degiskenlerini ayarla
    local sb_url=""
    local sb_anahtar=""
    if [[ -f "${BORSA_KLASORU}/veritabani/supabase.ayarlar.sh" ]]; then
        sb_url=$(grep -m1 '_SUPABASE_URL=' \
            "${BORSA_KLASORU}/veritabani/supabase.ayarlar.sh" \
            | cut -d= -f2 | tr -d '"' | tr -d "'")
        sb_anahtar=$(grep -m1 '_SUPABASE_ANAHTAR=' \
            "${BORSA_KLASORU}/veritabani/supabase.ayarlar.sh" \
            | cut -d= -f2 | tr -d '"' | tr -d "'")
    fi
    ...
    # Arka planda baslat
    SUPABASE_URL="${sb_url}" \
    SUPABASE_ANAHTAR="${sb_anahtar}" \
    nohup "$python_yol" "$_MUM_BIRLESTIRICI" \
        >> "/tmp/borsa/_takip/birlestirici.log" 2>&1 &

    local yeni_pid=$!
    echo "$yeni_pid" > "$_MUM_BIRLESTIRICI_PID"

    echo "Mum birlestirici baslatildi (PID: $yeni_pid)"
    echo "Log: /tmp/borsa/_takip/birlestirici.log"
    return 0
}

# -------------------------------------------------------
# mum_birlestirici_durdur
# Birlestiriciyi durdurur. Acik mumlar kaydedilir.
# -------------------------------------------------------
mum_birlestirici_durdur() {
    if [[ ! -f "$_MUM_BIRLESTIRICI_PID" ]]; then
        echo "Mum birlestirici calismiyir"
        return 0
    fi
    ...
    if kill -0 "$pid" 2>/dev/null; then
        ...
        echo "Mum birlestirici durduruluyor (PID: $pid)..."

        # Kapanmasini bekle (max 10 saniye)
        local bekle=0
        ...
        if kill -0 "$pid" 2>/dev/null; then
            ...
            echo "Mum birlestirici durduruldu"
        fi
    else
        echo "Mum birlestirici zaten durmus"
    fi
...
# -------------------------------------------------------
# mum_birlestirici_durum
# Birlestirici durumunu gosterir.
# -------------------------------------------------------
mum_birlestirici_durum() {
    if [[ ! -f "$_MUM_BIRLESTIRICI_PID" ]]; then
        echo "Durum: DURMUS"
    ...
    if [[ -n "$pid" ]] && kill -0 "$pid" 2>/dev/null; then
        ...
        # Tick dosya istatistikleri
        local tick_dizin="/tmp/borsa/_wss/tickler"
        if [[ -d "$tick_dizin" ]]; then
            local dosya_sayisi
            dosya_sayisi=$(find "$tick_dizin" -name "*.tick" -type f 2>/dev/null | wc -l)
            echo "Tick dosyasi: ${dosya_sayisi}"

            if [[ "$dosya_sayisi" -gt 0 ]]; then
                local toplam_boyut
                toplam_boyut=$(du -sh "$tick_dizin" 2>/dev/null | cut -f1)
                echo "Tick boyut: ${toplam_boyut}"
            fi
        fi

        # Log son satirlari
        local log="/tmp/borsa/_takip/birlestirici.log"
        if [[ -f "$log" ]]; then
            echo ""
            echo "Son log:"
            tail -5 "$log"
        ...
        echo "Durum: DURMUS (PID dosyasi eski)"
        rm -f "$_MUM_BIRLESTIRICI_PID"
    fi
    return 0
}
...
# =======================================================
# SEANS OTOMASYONU — CANLI VERI PIPELINE YONETIMI
# =======================================================
# Seans basladiginda REST polling + mum birlestirici otomatik
# baslatilir, seans sonunda otomatik durdurulur.
#
# Kullanim:
#   canli_veri_baslat      -> Takip listesi aktifse pipeline baslatir
#   canli_veri_durdur       -> Pipeline'i durdurur
#   canli_veri_durum        -> Polling + birlestirici durumu
#   canli_veri_seans_bekle  -> Seansi bekleyip otomatik baslatir/durdurur
# =======================================================

_CANLI_VERI_SEANS_PID="/tmp/borsa/_takip/seans_bekleyici.pid"

# -------------------------------------------------------
# canli_veri_baslat
# WSS (tercih) veya REST polling + mum birlestirici pipeline'ini baslatir.
# WSS destegi varsa REST polling baslatilmaz.
# Takip listesinde en az 1 sembol olmalidir.
# -------------------------------------------------------
canli_veri_baslat() {
    ...
    # Fiyat kaynagi kontrol
    if [[ -z "${_FIYAT_KAYNAGI_KURUM:-}" ]]; then
        echo "UYARI: Fiyat kaynagi ayarlanmamis"
        echo "Once bir oturuma giris yapin veya: fiyat_kaynagi_baslat"
        return 1
    fi

    # Tick dizinini hazirla
    mkdir -p "${_TAKIP_TICK_DIZIN:-/tmp/borsa/_wss/tickler}" 2>/dev/null

    # Veri kaynagi secimi: WSS > REST polling
    local veri_modu="REST"

    if type adaptor_wss_destekliyor_mu &>/dev/null \
       && adaptor_wss_destekliyor_mu 2>/dev/null; then
        # WSS destekleniyor — WSS ile basla
        if ! _ziraat_wss_aktif_mi 2>/dev/null; then
            echo "WSS baglantisi kuruluyor..."
            # Sembol listesini dosyaya yaz
            local hesap
            hesap=$(cekirdek_aktif_hesap 2>/dev/null)
            if [[ -n "$hesap" ]]; then
                local sembol_dosyasi="${_ZIRAAT_WSS_DIZIN:-/tmp/borsa/_wss}/ziraat_${hesap}.semboller"
                mkdir -p "${_ZIRAAT_WSS_DIZIN:-/tmp/borsa/_wss}" 2>/dev/null
            ...
            if _ziraat_wss_aktif_mi 2>/dev/null; then
                ...
                echo "UYARI: WSS baglantisi kurulamadi — REST polling'e geciyor"
            fi
        else
    ...
    # REST polling — sadece WSS yoksa veya basarisizsa
    if [[ "$veri_modu" == "REST" ]]; then
        if ! _canli_veri_polling_aktif_mi; then
            takip_polling_baslat
        else
            echo "REST polling zaten aktif"
        fi
    fi

    # 2) Mum birlestirici baslat
    if ! _canli_veri_birlestirici_aktif_mi; then
        mum_birlestirici_baslat
    else
        echo "Mum birlestirici zaten aktif"
    fi

    echo ""
    echo "Canli veri pipeline baslatildi"
    echo "  Takip: $sembol_sayisi sembol"
    if [[ "$veri_modu" == "WSS" ]]; then
        echo "  Mod: WebSocket (SignalR)"
        echo "  Sunucu: ${_ZIRAAT_WSS_SUNUCU:-veri.ziraatyatirim.com.tr}"
    else
        echo "  Mod: REST polling (${_TAKIP_ONBELLEK_SURESI:-3}sn aralik)"
    fi
    echo "  Kaynak: ${_FIYAT_KAYNAGI_KURUM:-?}"
    return 0
}

# -------------------------------------------------------
# canli_veri_durdur
# WSS + REST polling + mum birlestirici pipeline'ini durdurur.
# -------------------------------------------------------
canli_veri_durdur() {
    local durdu=0

    # 0) WSS baglantisini kapat
    if type adaptor_wss_kapat &>/dev/null \
       && _ziraat_wss_aktif_mi 2>/dev/null; then
        adaptor_wss_kapat
        durdu=1
    fi
    ...
    # 2) Birlestirici durdur (acik mumlari kaydeder)
    if _canli_veri_birlestirici_aktif_mi; then
        mum_birlestirici_durdur
        durdu=1
    fi
    ...
    if [[ "$durdu" -eq 0 ]]; then
        echo "Canli veri pipeline zaten durmus"
    else
        echo "Canli veri pipeline durduruldu"
    fi
    return 0
}
...
# -------------------------------------------------------
canli_veri_durum() {
    echo "=== Canli Veri Pipeline Durumu ==="
    echo ""
    ...
    if [[ -n "$semboller" ]]; then
        local sembol_sayisi
        sembol_sayisi=$(echo "$semboller" | wc -l)
        echo "Takip listesi: $sembol_sayisi sembol"
    else
        echo "Takip listesi: BOS"
    fi
    echo ""
    ...
    if _canli_veri_polling_aktif_mi; then
        local polling_pid
        polling_pid=$(cat "${_TAKIP_DIZIN:-/tmp/borsa/_takip}/polling.pid" 2>/dev/null)
        echo "Durum: AKTIF (PID: $polling_pid)"
        echo "Aralik: ${_TAKIP_ONBELLEK_SURESI:-3}sn"
        echo "Kaynak: ${_FIYAT_KAYNAGI_KURUM:-?}"
    else
        echo "Durum: DURMUS"
    fi
    ...
    # WSS durumu
    echo "--- WebSocket (SignalR) ---"
    if type adaptor_wss_durum &>/dev/null; then
        if _ziraat_wss_aktif_mi 2>/dev/null; then
            adaptor_wss_durum
        else
            echo "Durum: KAPALI"
        ...
        echo "Durum: DESTEKLENMIYOR"
    fi
    echo ""

    # Birlestirici durumu
    echo "--- Mum Birlestirici ---"
    if _canli_veri_birlestirici_aktif_mi; then
        mum_birlestirici_durum
    else
        echo "Durum: DURMUS"
    ...
    # Tick dosyasi istatistikleri
    local tick_dizin="${_TAKIP_TICK_DIZIN:-/tmp/borsa/_wss/tickler}"
    if [[ -d "$tick_dizin" ]]; then
        local tick_sayisi
        tick_sayisi=$(find "$tick_dizin" -name "*.tick" -newer "$tick_dizin" \
            -mmin -5 2>/dev/null | wc -l)
        echo "Son 5dk aktif tick dosyasi: $tick_sayisi"
    fi

    # Seans bekleyici
    if [[ -f "$_CANLI_VERI_SEANS_PID" ]]; then
        local seans_pid
        seans_pid=$(cat "$_CANLI_VERI_SEANS_PID" 2>/dev/null)
...
# Arka planda calisir, sabah seans acilinca pipeline
# baslatir, aksam kapaninca durdurur.
# -------------------------------------------------------
canli_veri_seans_bekle() {
    # Zaten calisiyor mu
    if [[ -f "$_CANLI_VERI_SEANS_PID" ]]; then
        local eski_pid
        eski_pid=$(cat "$_CANLI_VERI_SEANS_PID" 2>/dev/null)
    ...
    echo "  Seans acilinca pipeline otomatik baslar"
    echo "  Seans kapaninca pipeline otomatik durur"
    return 0
}
...
canli_veri_seans_durdur() {
    ...
    local pid
    pid=$(cat "$_CANLI_VERI_SEANS_PID" 2>/dev/null)
    rm -f "$_CANLI_VERI_SEANS_PID"
...
_canli_veri_seans_dongusu() {
    ...
    while true; do
        ...
        if bist_seans_acik_mi 2>/dev/null; then
            # Seans acik — pipeline baslatilmamissa baslat
            if [[ "$pipeline_acik" -eq 0 ]]; then
                _cekirdek_log "Seans acildi — canli veri pipeline baslatiliyor"
                canli_veri_baslat > /dev/null 2>&1 || true
                pipeline_acik=1
            fi
        else
            # Seans kapali — pipeline aciksa durdur
            if [[ "$pipeline_acik" -eq 1 ]]; then
                _cekirdek_log "Seans kapandi — canli veri pipeline durduruluyor"
...
# -------------------------------------------------------
# _canli_veri_polling_aktif_mi (dahili)
# Polling dongusunun calisip calismadigini kontrol eder.
# -------------------------------------------------------
_canli_veri_polling_aktif_mi() {
...
# -------------------------------------------------------
# _canli_veri_birlestirici_aktif_mi (dahili)
# Mum birlestirici prosesinin calisip calismadigini kontrol eder.
# -------------------------------------------------------
_canli_veri_birlestirici_aktif_mi() {
    if [[ -f "$_MUM_BIRLESTIRICI_PID" ]]; then
        local pid
        pid=$(cat "$_MUM_BIRLESTIRICI_PID" 2>/dev/null)
        [[ -n "$pid" ]] && kill -0 "$pid" 2>/dev/null && return 0
